...
/*******************************************************************************
Additional BigQueryConnection Class Information

 Because the DBConnector requires a java.sql.Connection, this seperate class is needed to return BigQuery connection
 Keeping the split between the connector and the rest of the functionality in case we want multiple connection types
 later on. Ex. Adding ODBC+JDBC along with the BQ API.
...
public class BigQueryConnection {
	...
	private BigQuery bq_connection;
	private boolean verbose = false;
	private static DecimalFormat decimalFormat = new DecimalFormat("#.#");
	private DbType dbType;

	public BigQueryConnection(String server, String domain, String user, String password, DbType dbType) {
		//field mappings:
		...
		this.bq_connection = BQConnector.connectToBigQuery(server, user, password);

		//should always be BigQuery. Just do a check.
		this.dbType = dbType;

	}

	public List<String> getBQTableNames(String database) {
		List<String> ret_names = new ArrayList<String>();
		ret_names.add("dummytable1");
		return ret_names;
	}
