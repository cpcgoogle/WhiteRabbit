...
import org.ohdsi.databases.BigQueryConnection;
...
public class WhiteRabbitMain implements ActionListener {
	...
	public final static String  BIG_QUERY_DISPLAY 				= "Google BigQuery";
	...
	private JPanel createLocationsPanel() {
		...
		sourceType = new JComboBox<String>(new String[] { "Delimited text files",  BIG_QUERY_DISPLAY, "MySQL", "Oracle", "SQL Server", "PostgreSQL", "MS Access", "PDW", "Redshift", "Teradata" });
		...
		sourceType.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent arg0) {
				...
				if (IS_BIGQUERY)
				{
					...
					if (!(!sourceIsFiles && arg0.getItem().toString().equals(BIG_QUERY_DISPLAY)))
					{
				...
				if (!sourceIsFiles && arg0.getItem().toString().equals("Oracle")) {
				...
				} else if (!sourceIsFiles && arg0.getItem().toString().equals(BIG_QUERY_DISPLAY)) {
					...
					sourceServerField.setText("https://www.googleapis.com/bigquery/v2:443");
					sourceServerField.setEditable(true);
...
//                    });
					...
					sourcePanel.add(new JLabel("Billing Project"));
					...
					//TODO: Remove these.
                    sourceUserField.setText("Crosbie-Test-Project");
                    sourceDatabaseField.setText("[google.com:crosbie-test-project:demos.nyc_weather]");
...
//		//itemStateChanged(arg_in);
	...
	private Image loadIcon(String name, JFrame f) {
		...
		//TODO: This line does not load the icon resource from the class unless I manually copy images into target.
		icon = Toolkit.getDefaultToolkit().getImage(WhiteRabbitMain.class.getResource(name));
	...
	private DbSettings getSourceDbSettings() {
		...
		if (sourceType.getSelectedItem().equals("Delimited text files")) {
			...
			if (sourceType.getSelectedItem().toString().equals("MySQL"))
				...
				dbSettings.dbType = DbType.TERADATA;
			else if (sourceType.getSelectedItem().toString().equals(BIG_QUERY_DISPLAY)) {
				dbSettings.dbType = DbType.BIGQUERY;
				//overide the domain with dataset to avoid modifying RichConnection signature
...
//			{
//				msg("testing some BQ");
//				BigQueryConnection bq_con;
//				try {
//					bq_con = new BigQueryConnection(dbSettings.server, dbSettings.domain, dbSettings.user, dbSettings.password, dbSettings.dbType);
//				} catch (Exception e) {
//					String message = "Could not connect to BigQuery: " + e.getMessage();
//					JOptionPane.showMessageDialog(frame, StringUtilities.wordWrap(message, 80), "Error connecting to BigQuery", JOptionPane.ERROR_MESSAGE);
//					return;
//				}
//
//
...
//			}else {
			...
			try {
				...
				if (IS_BIGQUERY)
				{
					message = "Could not connect to BigQuery: " + e.getMessage();
				}
				JOptionPane.showMessageDialog(frame, StringUtilities.wordWrap(message, 80), "Error connecting to server", JOptionPane.ERROR_MESSAGE);
				return;
			}
	...
	private DbSettings getTargetDbSettings() {
		...
		if (targetType.getSelectedItem().equals("Delimited text files")) {
			...
			else if (sourceType.getSelectedItem().toString().equals("Google BigQuery"))
				{
					msg("testing BQ");
				}
			}
